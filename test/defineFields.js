...
describe('define', function () {
  const fields = [{
    ...
    length: 32,
    allowLess: true,
    default: eleUtil.zeros(32)
  }]

  it('should trim zeros', function () {
    var someOb = {}
    eleUtil.defineProperties(someOb, fields)
      // Define Properties
    someOb.r = '0x00004'
  ...
  it('shouldn\'t allow wrong size for exact size requirements', function () {
    ...
    assert.throws(function () {
      ...
      eleUtil.defineProperties(someOb, tmp)
    })
  })
  ...
  it('it should accept rlp encoded intial data', function () {
    ...
    eleUtil.defineProperties(someOb, fields, data)
    ...
    eleUtil.defineProperties(someOb2, fields, rlpEncoded)
    assert.equal(someOb2.serialize().toString('hex'), rlpEncoded, 'the constuctor should accept rlp encoded buffers')

    var someOb3 = {}
    eleUtil.defineProperties(someOb3, fields, expectedArray)
  ...
  it('it should not accept invalid values in the constuctor', function () {
    var someOb = {}
    assert.throws(function () {
      eleUtil.defineProperties(someOb, fields, 5)
    }, 'should throw on nonsensical data')

    assert.throws(function () {
      eleUtil.defineProperties(someOb, fields, Array(6))
