...
describe('sha3-512', function () {
  it('should produce a sha3', function () {
    ...
    var hash = eleUtils.sha3(msg, 512)
    assert.equal(hash.toString('hex'), r)
  })
})
...
describe('ripemd160', function () {
  it('should produce a ripemd160', function () {
    ...
    var hash = eleUtils.ripemd160(msg)
    assert.equal(hash.toString('hex'), r)
  })
  ...
  it('should produce a padded ripemd160', function () {
    ...
    var hash = eleUtils.ripemd160(msg, true)
    assert.equal(hash.toString('hex'), r)
  })
})
...
describe('bufferToHex', function () {
  it('should convert a buffer to hex', function () {
    ...
    var hex = eleUtils.bufferToHex(buf)
...
describe('intToBuffer', function () {
  it('should convert a int to a buffer', function () {
    var i = 6003400
    var buf = eleUtils.intToBuffer(i)
...
describe('bufferToInt', function () {
  it('should convert a int to hex', function () {
    var buf = new Buffer('5b9ac8', 'hex')
    var i = eleUtils.bufferToInt(buf)
    assert.equal(i, 6003400)
    assert.equal(eleUtils.bufferToInt(new Buffer([])), 0)
  })
...
describe('fromSigned', function () {
  it('should convert an unsigned (negative) buffer to a singed number', function () {
    ...
    assert.equal(eleUtils.fromSigned(buf), neg)
  })
...
describe('toUnsigned', function () {
  it('should convert a signed (negative) number to unsigned', function () {
    ...
    assert.equal(eleUtils.toUnsigned(num).toString('hex'), hex)
  })
...
describe('isValidPrivate', function () {
  var SECP256K1_N = new eleUtils.BN('fffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141', 16)
  it('should fail on short input', function () {
    var tmp = '0011223344'
    assert.equal(eleUtils.isValidPrivate(new Buffer(tmp, 'hex')), false)
  })
  ...
  it('should work otherwise (< N)', function () {
    ...
    assert.equal(eleUtils.isValidPrivate(new Buffer(tmp, 'hex')), true)
  })
})
...
describe('isValidPublic', function () {
  it('should fail on too short input', function () {
    ...
    assert.equal(eleUtils.isValidPublic(pubKey), false)
  })
  it('should fail on too big input', function () {
  ...
  it('shouldn\'t fail on SEC1 key with sanitize enabled', function () {
    var pubKey = '043a443d8381a6798a70c6ff9304bdc8cb0163c23211d11628fae52ef9e0dca11a001cf066d56a8156fc201cd5df8a36ef694eecd258903fca7086c1fae7441e1d'
    pubKey = new Buffer(pubKey, 'hex')
    assert.equal(eleUtils.isValidPublic(pubKey, true), true)
  })
  it('should fail with an invalid SEC1 public key', function () {
    ...
    pubKey = new Buffer(pubKey, 'hex')
    assert.equal(eleUtils.isValidPublic(pubKey, true), false)
  })
  ...
  it('should work otherwise', function () {
    ...
    pubKey = new Buffer(pubKey, 'hex')
    assert.equal(eleUtils.isValidPublic(pubKey), true)
  })
})

describe('importPublic', function () {
  var pubKey = '3a443d8381a6798a70c6ff9304bdc8cb0163c23211d11628fae52ef9e0dca11a001cf066d56a8156fc201cd5df8a36ef694eecd258903fca7086c1fae7441e1d'
  it('should work with an Elementrem public key', function () {
    var tmp = '3a443d8381a6798a70c6ff9304bdc8cb0163c23211d11628fae52ef9e0dca11a001cf066d56a8156fc201cd5df8a36ef694eecd258903fca7086c1fae7441e1d'
    assert.equal(eleUtils.importPublic(new Buffer(tmp, 'hex')).toString('hex'), pubKey)
  })
...
describe('publicToAddress', function () {
  it('should produce an address given a public key', function () {
    ...
    pubKey = new Buffer(pubKey, 'hex')
    var r = eleUtils.publicToAddress(pubKey)
  ...
  it('should produce an address given a SEC1 public key', function () {
    ...
    var address = '2f015c60e0be116b1f0cd534704db9c92118fb6a'
    pubKey = new Buffer(pubKey, 'hex')
    var r = eleUtils.publicToAddress(pubKey, true)
    assert.equal(r.toString('hex'), address)
  })
  it('shouldn\'t produce an address given an invalid SEC1 public key', function () {
    ...
    assert.throws(function () {
      eleUtils.publicToAddress(pubKey, true)
    })
  })
  it('shouldn\'t produce an address given an invalid public key', function () {
    ...
    pubKey = new Buffer(pubKey, 'hex')
    assert.throws(function () {
      eleUtils.publicToAddress(pubKey)
    })
  })
})
...
describe('privateToPublic', function () {
  it('should produce a public key given a private key', function () {
    ...
    var r = eleUtils.privateToPublic(privateKey).toString('hex')
  ...
  it('shouldn\'t produce a public key given an invalid private key', function () {
    ...
    assert.throws(function () {
      eleUtils.privateToPublic(privateKey1)
    })
    assert.throws(function () {
      eleUtils.privateToPublic(privateKey2)
    })
  })
})
...
describe('privateToAddress', function () {
  it('should produce an address given a private key', function () {
    ...
    // Our private key
    var privateKey = new Buffer([234, 84, 189, 197, 45, 22, 63, 136, 201, 58, 176, 97, 87, 130, 207, 113, 138, 46, 251, 158, 81, 167, 152, 154, 171, 27, 8, 6, 126, 156, 28, 95])
    var r = eleUtils.privateToAddress(privateKey).toString('hex')
    assert.equal(r.toString('hex'), address)
  })
})
...
describe('generateAddress', function () {
  it('should produce an address given a public key', function () {
    var add = eleUtils.generateAddress('990ccf8a0de58091c028d6ff76bb235ee67c1c39', 14).toString('hex')
...
describe('generateAddress with hex prefix', function () {
  it('should produce an address given a public key', function () {
    var add = eleUtils.generateAddress('0x990ccf8a0de58091c028d6ff76bb235ee67c1c39', 14).toString('hex')
...
describe('generateAddress with nonce 0 (special case)', function () {
  it('should produce an address given a public key', function () {
    var add = eleUtils.generateAddress('0x990ccf8a0de58091c028d6ff76bb235ee67c1c39', 0).toString('hex')
...
describe('hex prefix', function () {
  ...
  it('should add', function () {
    assert.equal(eleUtils.addHexPrefix(string), '0x' + string)
  })
  it('should return on non-string input', function () {
    assert.equal(eleUtils.addHexPrefix(1), 1)
  })
})

describe('isPrecompiled', function () {
  it('should return true', function () {
    assert.equal(eleUtils.isPrecompiled('0000000000000000000000000000000000000001'), true)
    assert.equal(eleUtils.isPrecompiled('0000000000000000000000000000000000000002'), true)
    assert.equal(eleUtils.isPrecompiled('0000000000000000000000000000000000000003'), true)
    assert.equal(eleUtils.isPrecompiled('0000000000000000000000000000000000000004'), true)
  })
  it('should return false', function () {
    assert.equal(eleUtils.isPrecompiled('0000000000000000000000000000000000000000'), false)
    assert.equal(eleUtils.isPrecompiled('0000000000000000000000000000000000000005'), false)
    assert.equal(eleUtils.isPrecompiled('1000000000000000000000000000000000000000'), false)
  })
})
...
describe('toBuffer', function () {
  it('should work', function () {
    // Buffer
    assert.deepEqual(eleUtils.toBuffer(new Buffer([])), new Buffer([]))
    // Array
    assert.deepEqual(eleUtils.toBuffer([]), new Buffer([]))
    // String
    assert.deepEqual(eleUtils.toBuffer('11'), new Buffer([49, 49]))
    assert.deepEqual(eleUtils.toBuffer('0x11'), new Buffer([17]))
    assert.deepEqual(eleUtils.toBuffer('1234').toString('hex'), '31323334')
    assert.deepEqual(eleUtils.toBuffer('0x1234').toString('hex'), '1234')
    // Number
    assert.deepEqual(eleUtils.toBuffer(1), new Buffer([1]))
    // null
    assert.deepEqual(eleUtils.toBuffer(null), new Buffer([]))
    // undefined
    assert.deepEqual(eleUtils.toBuffer(), new Buffer([]))
    // 'toArray'
    assert.deepEqual(eleUtils.toBuffer(new BN(1)), new Buffer([1]))
  })
  it('should fail', function () {
    assert.throws(function () {
      eleUtils.toBuffer({ test: 1 })
    })
  })
})
...
describe('baToJSON', function () {
  it('should turn a array of buffers into a pure json object', function () {
    ...
    assert.deepEqual(eleUtils.baToJSON(ba), ['0x00', '0x01', ['0x02']])
  })
  it('should turn a buffers into string', function () {
    assert.deepEqual(eleUtils.baToJSON(new Buffer([0])), '0x00')
  })
})
...
describe('ecsign', function () {
  it('should produce a signature', function () {
    var sig = eleUtils.ecsign(echash, ecprivkey)
...
describe('ecrecover', function () {
  it('should recover a public key', function () {
    ...
    var pubkey = eleUtils.ecrecover(echash, 27, r, s)
    assert.deepEqual(pubkey, eleUtils.privateToPublic(ecprivkey))
  })
  it('should fail on an invalid signature (v = 21)', function () {
    var r = new Buffer('99e71a99cb2270b8cac5254f9e99b6210c6c10224a1579cf389ef88b20a1abe9', 'hex')
    var s = new Buffer('129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca66', 'hex')
    assert.throws(function () {
      eleUtils.ecrecover(echash, 21, r, s)
    })
  })
  it('should fail on an invalid signature (v = 29)', function () {
    ...
    var s = new Buffer('129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca66', 'hex')
    assert.throws(function () {
      eleUtils.ecrecover(echash, 29, r, s)
    })
  })
  it('should fail on an invalid signature (swapped points)', function () {
    ...
    var s = new Buffer('129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca66', 'hex')
    assert.throws(function () {
      eleUtils.ecrecover(echash, 27, s, r)
    })
  })
})
...
describe('.toChecksumAddress()', function () {
  it('should work', function () {
    for (var i = 0; i < checksumAddresses.length; i++) {
      ...
      assert.equal(eleUtils.toChecksumAddress(tmp.toLowerCase()), tmp)
    }
  })
})
...
describe('.isValidChecksumAddress()', function () {
  it('should return true', function () {
    for (var i = 0; i < checksumAddresses.length; i++) {
      assert.equal(eleUtils.isValidChecksumAddress(checksumAddresses[i]), true)
    }
  })
  it('should validate', function () {
    assert.equal(eleUtils.isValidChecksumAddress('0x2f015c60e0be116b1f0cd534704db9c92118fb6a'), false)
  })
})

describe('.isValidAddress()', function () {
  it('should return true', function () {
    assert.equal(eleUtils.isValidAddress('0x2f015c60e0be116b1f0cd534704db9c92118fb6a'), true)
    assert.equal(eleUtils.isValidAddress('0x52908400098527886E0F7030069857D2E4169EE7'), true)
  })
  it('should return false', function () {
    assert.equal(eleUtils.isValidAddress('2f015c60e0be116b1f0cd534704db9c92118fb6a'), false)
    assert.equal(eleUtils.isValidAddress('0x2f015c60e0be116b1f0cd534704db9c92118fb6'), false)
    assert.equal(eleUtils.isValidAddress('0x2f015c60e0be116b1f0cd534704db9c92118fb6aa'), false)
    assert.equal(eleUtils.isValidAddress('x2f015c60e0be116b1f0cd534704db9c92118fb6a'), false)
  })
})
...
describe('message sig', function () {
  ...
  it('should return hex strings that the RPC can use', function () {
    assert.equal(eleUtils.toRpcSig(27, r, s), '0x99e71a99cb2270b8cac5254f9e99b6210c6c10224a1579cf389ef88b20a1abe9129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca6600')
    assert.deepEqual(eleUtils.fromRpcSig('0x99e71a99cb2270b8cac5254f9e99b6210c6c10224a1579cf389ef88b20a1abe9129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca6600'), {
      v: 27,
      r: r,
      s: s
    })
